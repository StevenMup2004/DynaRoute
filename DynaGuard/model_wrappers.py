...
#             prediction_logits = logits[batch_indices, last_token_indices, :]
#             prediction_probs = torch.nn.functional.softmax(prediction_logits, dim=-1)

#             for row_idx in range(prediction_logits.shape[0]):
#                 row_logits = prediction_logits[row_idx]
#                 row_probs = prediction_probs[row_idx]
#                 pos_prob = row_probs[pos_token_id].item()
#                 neg_prob = row_probs[neg_token_id].item()
#                 pos_logit = row_logits[pos_token_id].item()
#                 neg_logit = row_logits[neg_token_id].item()
#                 prob_pairs.append((pos_prob, neg_prob))
#                 logit_pairs.append((pos_logit, neg_logit))
...
#         outputs = []
#         batch_size = self.batch_size
#         for start in tqdm(range(0, len(messages), batch_size), desc="HF generation batches"):
#             batch_messages = messages[start:start + batch_size]
#             inputs = self.tokenizer(batch_messages, return_tensors="pt", padding=True).to(self.model.device)
#             with torch.no_grad():
#                 output_content = self.model.generate(
...
#                     for offset, output_text in enumerate(batch_outputs):
#                         record = {
...
#                             record["metadata"] = stream_metadata[start + offset]
...
#                 "vllm is not installed, so VllmModelWrapper cannot be used. "
#                 "Run with --no-use_vllm to use HfModelWrapper, or install vllm if you need VLLM."
#             )
#         super().__init__(model_name, temperature, top_k, top_p, min_p, max_new_tokens, custom_name)
...
class HfModelWrapper(LocalModelWrapper):
    ...
    def get_hidden_state_features(self, messages, layer_idx=-1):
        ...
        This follows the same extraction pattern as safe-route:
        ``output_hidden_states=True`` -> ``hidden_states[layer_idx]`` ->
        ``[:, -1, :]``.
        """
        batch_size = self.batch_size
        features = []
